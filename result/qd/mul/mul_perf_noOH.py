...
def load_data(filename):
    ...
    with open(filename, 'r') as fi:
        for line in fi.readlines():
            ...
            data_x.append(int(x))
            data_y1.append(171.0 / 192.0 * float(cycle1))
            data_y2.append(171.0 / 192.0 * float(cycle2))
            data_y3.append(171.0 / 192.0 * float(cycle3))
            data_y4.append(171.0 / 192.0 * float(cycle4))
            data_y5.append(171.0 / 192.0 * float(cycle5))
            data_y6.append(171.0 / 192.0 * float(cycle6))
    return data_x, data_y1, data_y2, data_y3, data_y4, data_y5, data_y6

if __name__ == '__main__':
    data_x, data_y1, data_y2, data_y3, data_y4, data_y5, data_y6 = load_data('qd_mul_noOH_perf.data')
    ...
    plt.text(512*1.05, 5.5, 'L1 Cache', color='blue', fontsize='x-large')
    plt.axvline(x=4096, linewidth=2, color='blue')
    plt.text(4096*1.05, 5.5, 'L2 Cache', color='blue', fontsize='x-large')
    plt.axvline(x=98304, linewidth=2, color='blue')
    plt.text(98304*1.05, 5.5, 'L3 Cache', color='blue', fontsize='x-large')
    plt.axhline(y=8, linewidth=2, color='darkred')
    plt.text(512*1.05, 8.1, '100% Theoretical Maximum Performance (no FMA)', color='darkred', fontsize='x-large')
    ...
    plt.savefig('qd_mul_noOH_perf.pdf')
