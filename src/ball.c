...
int main() {
  ...
  for (int i = 0; i < (1 << 16); ++i) {
    ...
    // double lo = (double)rand() / RAND_MAX * 2.0 - 1.0;
    // double ro = (double)rand() / RAND_MAX * 2.0 - 1.0;
    // test_ball(lo, ro, BALL_ADD);
  }
  ...
  int precision = 30;
  BigFloat test1 = double_to_big_float_fixed_precision(testdata1,
  precision),test2 = double_to_big_float_fixed_precision(testdata2,precision);
  big_float_print(test1);
  ...
  // //BigFloat out2 = big_float_reciprocal_sqrt_fix_precision(bf1, precision,
  // 1e-10); BigFloat out3 = big_float_guided_reciprocal_sqrt_fix_precision(bf1,
  // precision);
