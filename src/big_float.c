...
BigFloat big_float_multiply(BigFloat lo, BigFloat ro) {
  ...
  if (ans.mantissa.data.size !=
      (lo.mantissa.data.size) + (ro.mantissa.data.size))
    --ans.power;
  if (0 == ans.mantissa.sign)
    ans.power = 0;
  return ans;
}

BigFloat big_float_mul_fixed_precision(BigFloat lo, BigFloat ro,
                                       const int precision) {
  assert(lo.mantissa.data.size >= precision &&
         ro.mantissa.data.size >= precision);
  BigFloat ans;
  ans.power = lo.power + ro.power;
  int powerdiff = 0;
  ans.mantissa = big_integer_multiply_fixed_precision(lo.mantissa, ro.mantissa,
                                                      precision, &powerdiff);
  ans.power += powerdiff;
  if (0 == ans.mantissa.sign)
    ans.power = 0;
...
void big_float_div_by_power_of_two_inplace_fixed_precision(
    BigFloat *bf, int power, const int precision) {
  assert(power >= 0);
  ...
  if (bf->mantissa.data.bits[bf->mantissa.data.size - 1] < threshold)
    bf->power--;
  big_integer_div_by_power_of_two_inplace_fixed_precision(&bf->mantissa, power,
                                                          precision);
}

void big_float_mul_toplace_fixed_precision(BigFloat lo, BigFloat ro,
                                           BigFloat *res, const int precision) {
  assert(lo.mantissa.data.size >= precision &&
         ro.mantissa.data.size >= precision);
  res->power = lo.power + ro.power;
  int powerdiff = 0;
  big_integer_multiply_toplace_fixed_precision(
      lo.mantissa, ro.mantissa, &res->mantissa, precision, &powerdiff);
  res->power += powerdiff;
  if (0 == res->mantissa.sign)
    res->power = 0;
}

void big_float_mul_inplace_fixed_precision(BigFloat *lo, BigFloat ro,
                                           const int precision) {
  lo->power = lo->power + ro.power;
  int powerdiff = 0;
  big_integer_multiply_inplace_fixed_precision(&lo->mantissa, ro.mantissa,
                                               precision, &powerdiff);
  lo->power += powerdiff;
  if (0 == lo->mantissa.sign)
    lo->power = 0;
}
...
BigFloat big_float_add(BigFloat lo, BigFloat ro) {
  ...
  int num_zeros_add_to_lo =
      lo.power - ro.power - lo.mantissa.data.size + ro.mantissa.data.size;
  BigInteger lo_man_aligned, ro_man_aligned;
  if (num_zeros_add_to_lo > 0) {
    lo_man_aligned =
        big_integer_add_trailing_zeros(lo.mantissa, num_zeros_add_to_lo);
    ...
    if (ans.mantissa.sign == 0)
      ans.power = 0;
    free(lo_man_aligned.data.bits);
  } else if (num_zeros_add_to_lo < 0) {
    lo_man_aligned = lo.mantissa;
    ro_man_aligned =
        big_integer_add_trailing_zeros(ro.mantissa, -num_zeros_add_to_lo);
    ans.mantissa = big_integer_add(lo_man_aligned, ro_man_aligned);
    ans.power = lo.power + ans.mantissa.data.size - lo_man_aligned.data.size;
    if (ans.mantissa.sign == 0)
      ans.power = 0;
...
void big_float_add_inplace_fixed_precision(BigFloat *lo, BigFloat ro,
                                           const int precision) {
  int carried = 0;
  ...
  if (num_zeros_add_to_lo > 0) {
    ...
    lo_man_aligned = &lo->mantissa;
    ro_man_aligned = ro.mantissa;
    big_integer_add_inplace_fixed_precision(lo_man_aligned, ro_man_aligned,
                                            precision, &carried);
...
void big_float_sub_inplace_fixed_precision(BigFloat *lo, BigFloat ro,
                                           const int precision) {
  ro.mantissa.sign *= -1;
...
BigFloat big_float_div(BigFloat lo, BigFloat ro) {
  ...
  tmp_point5.power = 0;
  unsigned int normalize_factor =
      (unsigned int)(log(ro.mantissa.data.bits[ro.mantissa.data.size - 1]) /
                     log(2)) +
      1;
  ...
  tmp_c1.mantissa.data.bits = (unsigned long *)calloc(4, sizeof(unsigned int));
  tmp_c2.mantissa.data.bits = (unsigned long *)calloc(4, sizeof(unsigned int));
  ...
  if (big_integer_compare_data(&(lo_man_aligned.data),
                               &(ro_man_aligned.data)) != 1) {
    --ans.power;
  }
  if (num_zeros_add_to_lo < 0) {
    free(lo_man_aligned.data.bits);
...
// BigFloat big_float_sqrt_fix_precision(BigFloat lo, const int precision,
//                                       const unsigned int x_0) {}

BigFloat big_float_reciprocal_sqrt_fix_precision(BigFloat lo,
                                                 const int precision,
                                                 double x_0) {
  // output precision = working precision / 2
  int working_prec;
  assert(precision > 1);
  if (lo.mantissa.data.size >= 2 * precision) {
    working_prec = 2 * precision;
  } else {
    working_prec = lo.mantissa.data.size;
  }
  ...
  unsigned int prev_value =
      x.mantissa.data.bits[x.mantissa.data.size - output_precision];
  // main loop
  for (int i = 0; i < num_Iter; i++) {
    ...
    if (x.mantissa.data.bits[x.mantissa.data.size - output_precision] ==
            prev_value &&
        i > min_iter) {
...
BigFloat big_float_guided_reciprocal_sqrt_fix_precision(BigFloat lo,
                                                        const int precision) {
  // output precision = working precision / 2
  int working_prec;
  assert(precision > 1);
